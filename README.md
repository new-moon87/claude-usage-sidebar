...
|---|---|---|
| **H** | Claude | 5시간 세션 한도 |
| **W** | Claude | 주간 한도 (전체 모델) |
| **F** | Claude | 주간 한도 (모델 전용 — Fable 등, 계정에 따라 이름이 바뀜) |
| **h** | Codex | 5시간대 한도 |
| **w** | Codex | 주간대 한도 |

Codex 알약은 소문자 + 초록 계열로 구분한다. 계정마다 한도 구성이 달라서, 창 길이가 24시간 이하인 것들과
...
`~/.codex/auth.json` 의 로그인 정보로 사용량을 물어보고, 안 되면 `~/.codex/sessions/` 의 CLI 기록을 읽는다.
기록은 Codex 를 쓰는 동안 실시간으로 쌓이므로 **사용량이 실제로 변하는 순간에는 최신**이고, 한동안 안 쓰면 낡는다.
낡은 값일 때는 패널에 `기록 N일 전` 이라고 같이 적는다. Codex 쪽 사용량 조회는 자주 하면 잠시 차단되므로
10분에 한 번만 물어본다.
